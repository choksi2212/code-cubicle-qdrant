...
fn payload_get(payload: &Payload, key: &str) -> Option<serde_json::Value> {
    match key {
        ...
        "cloudinary_public_id" => payload.cloudinary_public_id.clone().map(|v| serde_json::json!(v)),
        "cloudinary_tags" => Some(serde_json::json!(payload.cloudinary_tags)),
        "cloudinary_objects" => Some(serde_json::json!(payload.cloudinary_objects)),
        "cloudinary_ocr_text" => payload.cloudinary_ocr_text.clone().map(|v| serde_json::json!(v)),
